...
{
    [Area("Admin")]
    public class MenuGVController : Controller
    {
        private readonly DataContext _db;
        public MenuGVController(DataContext db) => _db = db;
        ...
        public async Task<IActionResult> Create(AdminMenu model)
        {
            ...
            // Không cho tự tham chiếu làm cha
        ...
        public async Task<IActionResult> Edit(AdminMenu model, string? q, int page = 1)
        {
            ...
            item.AreaName = "Giangvien"; // cố định khu vực GV
        ...
        private async Task BuildParentOptions(long? excludeId = null)
        {
            var parents = await _db.AdminMenus
                .AsNoTracking()
                .Where(x => x.MaVaiTro == 2 && (excludeId == null || x.AdminMenuID != excludeId.Value))
                .OrderBy(x => x.ParentLevel)
                .ThenBy(x => x.ItemLevel)
                .ThenBy(x => x.ItemName)
                .Select(x => new
